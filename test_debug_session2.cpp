...
int main() {
    std::cout << "Testing SAS -stdio with RUN statement..." << std::endl;
    ...
    std::cout << "Sending command with RUN..." << std::endl;

    // Send command followed by a DATA step with RUN to force processing
    fprintf(sas_stdin, "%%put hello world;\n");
    fprintf(sas_stdin, "DATA _null_; run;\n");
    ...
    std::cout << "Found marker: " << (found_marker ? "YES" : "NO") << std::endl;
